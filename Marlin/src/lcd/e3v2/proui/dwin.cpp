...
void EachMomentUpdate() {
  ...
  if (ELAPSED(ms, next_var_update_ms)) {
    ...
    #endif
    #if SHOW_TUNING_GRAPH
      if (checkkey == PidProcess) plot.Update((HMI_value.tempcontrol == PIDTEMP_START) ? thermalManager.wholeDegHotend(0) : thermalManager.wholeDegBed());
...
#if SHOW_TUNING_GRAPH && ANY(HAS_PID_HEATING, MPCTEMP)
...
#if ENABLED(SHOW_TUNING_GRAPH) && ENABLED(PLOT_TUNE_ITEM)
  #if HAS_TEMP_SENSOR
...
#if HAS_BED_PROBE
  void SetProbeOffsetX() { SetPFloatOnClick(-60, 60, UNITFDIGITS, nullptr); }
  void SetProbeOffsetY() { SetPFloatOnClick(-60, 60, UNITFDIGITS, nullptr); }
...
void Draw_FilSet_Menu() {
  checkkey = Menu;
  if (SET_MENU(FilSetMenu, MSG_FILAMENT_SET, 6)) {
...
//=============================================================================
  ...
  void UBLSmartFillMesh() {
    LOOP_L_N(x, GRID_MAX_POINTS_X) bedlevel.smart_fill_mesh();
  ...
  void Draw_MeshSet_Menu() {
    checkkey = Menu;
    if (SET_MENU(MeshMenu, MSG_MESH_LEVELING, 5)) {
      BACK_ITEM(Draw_AdvancedSettings_Menu);
      #if ENABLED(ACTIVATE_MESH_ITEM)
